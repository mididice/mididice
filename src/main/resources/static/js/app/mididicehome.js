...
$(document).on("mouseenter",".another_love",function(){
	...
	$(this).children().find('.pattern_img').attr('src', '/mididice/images/play_midi.png');
});
...
function animateDice(purpleDice, pinkDice) {
    ...
    var timer = setInterval(function(){
    	if(i<10){
    		$('.midi_dice_right').children().first().attr('src', '/mididice/images/dice/VioletDice01_0000'+i+'.png');	
    		$('.midi_dice_left').children().first().attr('src', '/mididice/images/dice/PinkDice01_0000'+i+'.png');
    	}else if(i<60){
        	$('.midi_dice_right').children().first().attr('src', '/mididice/images/dice/VioletDice01_000'+i+'.png');            
        	$('.midi_dice_left').children().first().attr('src', '/mididice/images/dice/PinkDice01_000'+i+'.png');            
        }else{
        	$('.midi_dice_right').children().first().attr('src', '/mididice/images/dice/VioletDice0'+purpleDice+'.png');
        	$('.midi_dice_left').children().first().attr('src', '/mididice/images/dice/PinkDice0'+pinkDice+'.png');
        	clearInterval(timer);
        	return;
        }
        i++;
...
var callback = function(midipath, pattern, seq){
	$('#patterns').append("<div class='another_love'><a href='#' id='samdasu' onClick='MIDIjs.play(\"/mididice/midi/"+midipath+"\");'>"
			+"<img src='/mididice/images/patterns/"
			...
			success:function(data){
				...
				for(var i = 0; i<obj.length;i++){
					...
					if(seq==bar-1){
						...
						$('.roll_dice_bottom').append("<span class='roll_btn' id='result'><img src='/mididice/images/result.png'></span>");
